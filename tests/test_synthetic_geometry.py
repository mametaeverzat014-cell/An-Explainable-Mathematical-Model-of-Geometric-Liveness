...
#: Расстояние от камеры до центра головы (условные единицы). Задаёт силу
#: перспективного эффекта: при D -> бесконечность проекция становится
#: ортографической и параллакс исчезает.
CAMERA_DISTANCE = 2.0
FOCAL = 1.0


def _canonical_face(nose_depth: float) -> dict[str, np.ndarray]:
    """Каноническая 3D-модель лица (X, Y, Z) в условных единицах.

    Args:
        nose_depth: вынос кончика носа вперёд. 0.0 => идеально плоский объект
            (модель распечатки или экрана).
    """
    return {
...
def _render_sequence(
    ...
    Используется ПЕРСПЕКТИВНАЯ проекция: точки ближе к камере проецируются с
    большим масштабом — именно это порождает параллакс, который измеряет модель.

    Колонки ``*_x``, ``*_y`` — координаты изображения; ``*_z`` — оценочная
    относительная псевдоглубина (аналог выхода MediaPipe), а НЕ измеренная
    физическая глубина.
    ...
    for frame_idx, (yaw, pitch) in enumerate(zip(yaw_degrees, pitch_degrees)):
        ...
        for name in LANDMARK_NAMES:
            ...
            # MediaPipe возвращает z примерно в том же масштабе, что и x, то есть
            # в координатах изображения: чем дальше лицо, тем меньше |z|.
            # Синтетическая модель обязана следовать этому же соглашению.
            row[f"{name}_z"] = z_rot * scale
        rows.append(row)
    return pd.DataFrame(rows)


def _head_motion(n_frames: int = 40) -> tuple[np.ndarray, np.ndarray]:
    """Комбинированное движение головы, предписанное протоколом съёмки."""
    return np.linspace(-25, 25, n_frames), np.linspace(-15, 15, n_frames)
